...
public class RunFlight {
                        ...
                        if (userType.equalsIgnoreCase("a")) { // Customer command / Purchasing flights
                            ...
                            while (!exitCustomerOptions) {
                                ...
                                if (customerOption.equalsIgnoreCase("a")) { // Customer command: purchase flight
                                    ...
                                    System.out.println("[B] Yes, I have the airport code");
                                    System.out.println("[C] No, please show me all flight info");
                                    String purchaseOption = scnr.nextLine(); 
                                    boolean rightID = false; 
                                    boolean rightDepartureDate = false;
                                    boolean rightOriginAirportCode = false; 
                                    boolean rightDepartureAirportCode = false; 
                                    ...
                                    while(!rightPurchaseOption) { // Check if right purchase option was selected (Has ID or Display flight Info)
                                        if (((purchaseOption).toLowerCase()).equals("a")) { // Knowing flight ID
                                            ...
                                            rightOriginAirportCode = true;
                                            rightDepartureAirportCode = true; 
                                            rightDepartureDate = true; 
                                        } // End of if statement for A - Knowing flight ID (Will ask for ID input later)

                                        if (((purchaseOption).toLowerCase()).equals("b")) { // Knowing airport codes
                                            rightPurchaseOption = true;   
                                        } // End of if statement for A - Knowing flight ID (Will ask for ID input later)

                                        if (((purchaseOption).toLowerCase()).equals("c")) {
                                    ...
                                    // Searching flight by airport codes
                                    // Origin airport code condition
                                    String originAirportCode = ""; 
                                    while(!rightOriginAirportCode) {
                                        System.out.println("Please provide origin airport code");
                                        originAirportCode = scnr.nextLine(); 

                                        // Calling method for verification of origin airport code
                                        rightOriginAirportCode = checkOriginCode(originAirportCode, flightFile);


                                        if (!rightOriginAirportCode) {
                                            System.out.println("Hmm seems like that code doesn't match any flights!");
                                            System.out.println("Please try again");
                                        }
                                        
                                    }

                                    // Departure airport code condition
                                    String departureAirportCode = ""; 
                                    while(!rightDepartureAirportCode) {
                                        System.out.println("Please provide departure airport code");
                                        departureAirportCode = scnr.nextLine(); 

                                        // Calling method for verification of origin airport code
                                        // Method checks for match of origin and departure + existence within map
                                        rightDepartureAirportCode = checkDepartureCode(originAirportCode, departureAirportCode, flightFile); 

                                        if (!rightDepartureAirportCode) {
                                            System.out.println("Hmm seems like that code doesn't match any flights!");
                                            System.out.println("Please try again");
                                        }
                                    ...
                                    // // Take in String from airport code -> ignore case -> find in map -> return boolean
                                    // checkOriginCode(originAirportCode);
                                    // // Take in String from airport code -> ignore case -> find in map -> return boolean
                                    // checkdepartureCode(originAirportCode, departureAirportCode);
                                    // checkDepartureDate(originAirportCode, departureAirportCode, departureDateOption);

                                    // Ask for optional departure date input
                                    String departureDateOption = "";  

                                    while (!rightDepartureDate) { 
                                        System.out.println("Do you want to enter your departure date?");
                                        System.out.println("[A] Yes");
                                        System.out.println("[B] No, please let me continue");
                                        departureDateOption = scnr.nextLine();

                                        if (departureDateOption.equalsIgnoreCase("a")) {
                                            System.out.println("Please enter departure date in format mm/dd/yy");
                                            System.out.println("Note: if your flight is in month or year below 10, enter without 0");
                                            System.out.println("E.g 3/23/23 instead of 03/23/23");
                                            departureDateOption = scnr.nextLine(); 

                                            // Check for matching codes (arrival and destination) along with given departure date
                                            rightDepartureDate = checkDepartureDate(originAirportCode, departureAirportCode, departureDateOption, flightFile); 
                                        }

                                        if (departureDateOption.equalsIgnoreCase("b")) {
                                            rightDepartureDate = true; 
                                        }
                                    }

                                    // Ask for ID (A option jumps directly, 
                                    // B asks for airport codes and optional departure date, 
                                    // C allows for display of flights and then asks for ID input)
                            ...
                            while (!exitEmployeeOptions) {
                                ...
                                System.out.println("[C] No actions, please log me out");
                                String purchaseOption = scnr.nextLine(); 
                                boolean rightPurchaseOption = false;  
                                ...
                                while(!rightPurchaseOption) { // Check if right purchase option was selected (Has ID or Display flight Info)
                                    ...
                                    if (purchaseOption.equalsIgnoreCase("c")) {
                                        rightPurchaseOption = true; 
                                    }
    ...
    public static String generateConfirmationNumber(Person currUser, int purchaseFlightID, String ticketType, int numTickets) {
        String confirmationNum = (currUser.getName()).charAt(0) + (currUser.getLastName()).charAt(0) + purchaseFlightID + "-" + ticketType.charAt(0) + numTickets; 
        return confirmationNum; 
    }

    // // Take in String from airport code -> ignore case -> find in map -> return boolean
    public static boolean checkOriginCode(String originAirportCode, HashMap<Integer, Flight> flightFile) {
        // Testing purposes -> Add iterating through HashMap of flightFile and find origin code
        return true; 
    }
    // // Take in String from airport code -> ignore case -> find in map -> return boolean
    public static boolean checkDepartureCode(String originAirportCode, String departureAirportCode, HashMap<Integer, Flight> flightFile) {
        // Testing purposes -> Add iterating through HashMap of flightFile and find origin code
        return true; 
    }
    // Take in both previous strings -> Find key with that data
    // checkDepartureDate(originAirportCode, departureAirportCode, departureDateOption);
    public static boolean checkDepartureDate(String originAirportCode, String departureAirportCode, String departureDateOption, HashMap<Integer, Flight> flightFile) {
        // Testing purposes -> Add iterating through HashMap of flightFile and find origin code
        return true; 
    }
